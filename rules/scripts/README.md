...
Core Python and R utilities that power the ATAC-seq pipeline's validation, quality control, analytics, and telemetry.
...
graph TD
    ...
    DAG --> DiffAcc[diff_accessibility.R]
    ...
    class TSS,PeakAnnot,DiffAcc rscript;
...
| `parse_qc_metrics.py` | After alignment | Evaluates FRiP, TSS enrichment, duplicate and mapping rates; flags failures |
...
| `geo_agent_bridge.py` | CLI tool | Imports GEO metadata, downloads raw FASTQs via SRA, and builds config/sample sheets |
| `atacseq_tool.py` | CLI/LangChain | LangChain agent tool wrapper to invoke the pipeline programmatically |
...
| `diff_accessibility.R` | After peak counting | Runs DESeq2 differential accessibility with diagnostics; handles < 10 peaks gracefully |
...
| `chromvar_analysis.R` | Motifs stage | Runs chromVAR motif accessibility scoring across consensus peaks |
| `cicero_analysis.R` | Single-cell mode | Computes co-accessibility peaks and cis-regulatory network maps using Cicero |
...
| `diff_accessibility.R` | < 10 consensus peaks | Writes dummy matrices + placeholder plots |
...
### 6. `geo_agent_bridge.py` (GEO Metadata Parser)
<details>
<summary>▶ Click to Expand Flowchart</summary>

```mermaid
graph TD
    A[GEO Metadata CSV] --> B(Parse accession IDs)
    B --> C{--download flag set?}
    C -- Yes --> D(Download FASTQ files via SRA Toolkit)
    C -- No --> E[Skip Download]
    D --> F(Generate config_geo.yaml & samples.tsv)
    E --> F
```

</details>

### 7. `atacseq_tool.py` (LangChain Wrapper)
<details>
<summary>▶ Click to Expand Flowchart</summary>

```mermaid
graph TD
    A[Agent Input Query] --> B(Determine command action)
    B --> C(Format parameters as CLI arguments)
    C --> D(Invoke Snakemake command via subprocess)
    D --> E[Parse outputs & return to agent]
```

</details>

### 8. `tss_enrichment.R` (TSS Coverage Engine)
<details>
<summary>▶ Click to Expand Flowchart</summary>
...
### 9. `diff_accessibility.R` (DESeq2 Accessibility contrast)
<details>
<summary>▶ Click to Expand Flowchart</summary>
