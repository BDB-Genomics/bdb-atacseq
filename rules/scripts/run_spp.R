...
if (!is.na(crosscorr$rel.phantom.coeff) && !is.nan(crosscorr$rel.phantom.coeff)) {
...
if (! is.na(iparams$output.plot.file)) {
	...
	title(main=get.file.parts(iparams$chip.file)$fullname,
	      sub=sprintf("NSC=%g,RSC=%g,Qtag=%s",crosscorr$phantom.coeff,crosscorr$rel.phantom.coeff,as.character(crosscorr$phantom.quality.tag)))
	dev.off();    
}
